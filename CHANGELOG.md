...
### 🚀 首页长文串联背诵核心入口与交互升级 (Home Screen Chained Recitation Featured Hub)
- **首页新增【长文串联 · 滚雪球背诵】专属首屏特辑卡片**：
  - 放置于“今日背诵”主界面核心视觉位，告别隐蔽层级菜单；
  - 深度阐释滚雪球递进逻辑（“1句 ➔ 1+2句 ➔ 1+2+3句”全篇连贯贯通，配合 FSRS 批量算法智能结算）；
  - 提供《劝学》、《师说》、《赤壁赋》、《阿房宫赋》、《琵琶行》等高频必背经典名篇一键直达快速标签；
  - 配备优雅的主操作按钮直接唤起全新篇目选择器。
- **新增全篇目串联选择器 (`ChainedArticleSelectionDialog`)**：
  - 支持按篇名、作者、朝代及名句正文即时模糊搜索；
  - 支持“全部篇目”、“高考必背72篇”、“当前教材”、“必修”与“选必”多维分类筛选；
  - 快速呈现篇目文体、朝代作者与开头名句摘要，点击即刻进入沉浸式滚雪球串诵。
- **全链路打通**：
  - `MainActivity` 与 `DashboardScreen` 全面连通 `onOpenSnowball` 路由，单元测试全部通过。
