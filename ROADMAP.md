...
- [ ] **构建 Room SQLite 离线体系**
  - 声明 `@Entity`：`ModuleEntity`、`ArticleEntity`、`FlashcardEntity`、`CardStateEntity`、`ReviewLogEntity`。
...
- [ ] **热力图无障碍 48dp 触摸靶达标**
  - 重构 [`RecitationHeatmapCard.kt`](file:///d:/OneDrive/Desktop/文言背诵/app/src/main/java/com/ancient/wenyan/ui/components/RecitationHeatmapCard.kt) 日历格点击，触摸区域由 15dp 扩大至标准 48dp，并加入平滑横向滚动与手势隔离，解决与根容器切 Tab 的手势冲突。
- [ ] **规范 Edge-to-Edge 边到边体验**
...
### 2.1 高考情境化理解性默写专项突破
- [ ] **新增情境化理解性默写题库**
  - 针对高考核心必背 72 篇，提供“真题题干 / 情境提示 ➔ 默写对应原句”模式。
  - 卡片正面展示情境考题（如：“《出师表》中诸葛亮劝诫后主不应妄自菲薄的句子是：____”），背面高亮核心考点与采分点。
- [ ] **专项易错考点库（通假字、古今异义、重点实虚词）**
  - 整理高考必考 120 实词、18 虚词及易错通假字。
  - 在翻卡背面展示字体演变与考点解析，在遮挡背诵模式中支持“核心考点自动精准挖空”。

### 2.2 端侧离线智能语音默写与 ASR 评测
- [ ] **引入端侧轻量语音识别引擎**
  - 支持学生闭眼朗诵背诵，系统毫秒级比对原文。
  - 实时可视化标出“漏字”、“倒装”、“错读”，提供声调、停顿与语速的精准反馈。

### 2.3 长篇诗文“滚雪球”串联背诵 (Snowball Paragraph Chaining)
- [ ] **长文渐进串联记忆模式**
  - 针对《离骚》、《赤壁赋》、《滕王阁序》、《蜀道难》等长文，提供渐进式上下文递增训练：
    - 阶段 1：背诵第 1 联
    - 阶段 2：遮挡连背 1+2 联
    - 阶段 3：遮挡连背 1+2+3 联
...
- [ ] **重构 FSRSOptimizer 寻优算法**
  - 废弃当前仅调优 7 个参数的经验规则与 5x5 网格搜索。
  - 引入纯 Kotlin 实现的 Nelder-Mead 多维单纯形寻优算法，基于全量复习日志计算 Log Loss（二元交叉熵损失），对全部 19 个参数实现真正的联合调优。
- [ ] **消除并发调优竞态**
  - 引入 `Mutex` 互斥保护，防止用户快速连续翻卡时启动多个优化协程发生乱序写入。

### 3.2 顽固漏卡 (Leech) 自动熔断与专项攻克
- [ ] **智能识别 Leech 卡片**
  - 统计卡片累计 `lapses`（遗忘次数）。当单张卡片累计遗忘达到阈值（如连续 4 次评分 Again）时，自动标记为“Leech 顽固卡”。
  - 暂停在常规队列中频繁刷屏，单独归入“难点攻坚错题集”，提示重点复习字词句义。
...
- [ ] **支持 WebDAV / 坚果云自动同步**
  - 支持学生在手机、折叠屏、平板及墨水屏设备之间无缝流转复习进度与打卡热力图。
  - 提供本地全量数据一键导出与还原 `.zip` 加密备份。

### 4.2 结业文牒成就海报与硬笔楷书字帖导出
- [ ] **国风“结业文牒”成就海报**
  - 单篇或整册文言文达到 100% 掌握时，生成带有传统朱砂印章、宣纸底纹与毛笔题字的高分辨率结业文牒，增强成就感。
- [ ] **硬笔书法田字格/米字格字帖导出**
  - 支持将背诵篇目一键导出为带拼音/注音的米字格默写练习 PDF，供学生打印临摹与手写练习。
