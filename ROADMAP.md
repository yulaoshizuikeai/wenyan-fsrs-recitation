...
### 4.3 Android 桌面微件与自适应折叠屏大屏适配 (Ecosystem & Adaptive Layout)
- [x] **Android 桌面微件 (AppWidget)**
  - 桌面直显“连胜火焰”、“今日剩余待复习句数”以及“今日精选名句”，点击直达复习队列。背诵进度实时联动刷新。
- [x] **自适应折叠屏与平板大屏布局**
  - 支持 `NavigationRail` 与响应式侧栏，大屏与展开折叠屏自适应布局（屏幕宽度 >= 600dp 自动启用导航侧轨），全方位兼容移动端与桌面/平板形态。
